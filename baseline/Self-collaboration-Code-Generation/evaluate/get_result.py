...
def run(base_dir, k, it=0):
            ...
            with jsonlines.open(eval_out_file) as jrp:
                for sample in jrp:
                    ...
                    if uid not in tmp_dict:
                        tmp_dict[uid] = {"correct": 0, "ut_reses": [], "src_uid": sample["src_uid"]}
    ...
    if it:
        ...
        output_file = os.path.join(output_dir, f"results_{now.tm_year}_{now.tm_mon}_{now.tm_mday}_{now.tm_hour+8}_{now.tm_min}.json")
