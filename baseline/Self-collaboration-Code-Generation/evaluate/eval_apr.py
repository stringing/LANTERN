...
def process(args):
    ...
    unit_tests = json.loads(sample["test"])
    compiler = LANG_CLUSTER_TO_LANG_COMPILER[sample["lang"]]
...
def run(base_dir):
    for k, debug_compiler in LANG_CLUSTER_TO_LANG_COMPILER.items():
        output_file = os.path.join(base_dir, f"{debug_compiler}.jsonl")
        with jsonlines.open(output_file, "w") as jwp:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=129
            ) as thread_executor:
                path_to_eval = os.path.join(base_dir, 'output')
